...
type NoteListProp = {
    ...
    onUpdateTag: (noteId: string, label: string) => void
...
export const NoteList = ({ availableTags, notes, onDeleteTag, onUpdateTag }:NoteListProp) => {
  ...
  return (
      <>
          ...
          <Row xs={1} m={2} lg={3} xl={4} className="g-3">
          ...
          <EditTagsModal show={showEditTagsModal} handleClose={() => setShowEditTagsModal(false)} onUpdateTag={onUpdateTag} onDeleteTag={onDeleteTag} availableTags={availableTags} />
      </>
  )
}
        ...
        <Card.Body>
            <Stack gap={2} className="align-items-center justify-content-center h-100">
                ...
                {tags.length > 0 && (<Stack gap={1} direction="horizontal" className="justify-content-center flex-wrap">
                    {tags.map(tag => (
                        <Badge key={tag.id} className="text-truncate badge badge-primary" >{tag.label}</Badge>
                    ))}
                </Stack>)}
            </Stack>
        </Card.Body>
